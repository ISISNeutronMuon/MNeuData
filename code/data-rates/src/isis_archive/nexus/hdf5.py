...
# Root NXentry group name in ISIS NeXus files.
NEXUS_ROOT_ENTRY = "raw_data_1"


def read_total_counts(entry: h5py.Group, nx_class: str, dataset_name: str) -> int:
...
def summarise_logs(parent: h5py.Group, group_name: str) -> tuple[int, int]:
...
def summarise_nexus(nexus_path: Path) -> NexusSummary:
    ...
    with h5py.File(str(nexus_path), "r") as fp:
        root_entry: h5py.Group = fp[NEXUS_ROOT_ENTRY]  # type: ignore
        return NexusSummary(
            total_detector_mevents=read_total_counts(root_entry, "NXdata", "counts")
            / 1_000_000,
            total_monitor_mevents=read_total_counts(root_entry, "NXmonitor", "data")
            / 1_000_000,
            **dict(
                zip(
                    (
                    ...
                    summarise_logs(root_entry, "selog")
                    + summarise_logs(root_entry, "framelog"),
                )
            ),
        )
